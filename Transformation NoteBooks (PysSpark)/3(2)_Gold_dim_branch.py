...
# MAGIC %md
# MAGIC # Creating Branch Dimension
...
df_src = spark.sql('''
                   select Distinct Branch_ID, BranchName from parquet.`abfss://silver@dlcarsalesproject.dfs.core.windows.net/carsales`
                   ''')
df_src.display()
...
# MAGIC %md
# MAGIC ### Dim_branch sink intial and increamental load (Just Bring Schema if table does not exists)
...
if spark.catalog.tableExists('cars_catalog.gold.dim_branch'):
    ...
    # Intial Load
    df_sink= spark.sql('''
                    select 1 as dim_branch_key, Branch_ID, BranchName from parquet.`abfss://silver@dlcarsalesproject.dfs.core.windows.net/carsales`
                        where 1=0
                        ''')
...
if spark.catalog.tableExists('cars_catalog.gold.dim_branch'):
 #Incremental Run
 delta_tbl = DeltaTable.forPath(spark, "abfss://gold@dlcarsalesproject.dfs.core.windows.net/dim_branch")
 delta_tbl.alias('trg').merge(df_final.alias('src'), 'trg.dim_branch_key = src.dim_branch_key')\
  ...
  df_final.write.format('delta')\
      .mode('overwrite')\
        .option("path", "abfss://gold@dlcarsalesproject.dfs.core.windows.net/dim_branch")\
